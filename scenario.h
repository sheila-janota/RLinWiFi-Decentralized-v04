...
{
    float delta = simulationTime/(this->nWifim-1);
    ...
    if (this->nWifim > 1)
    {
        ...
        for (int i = 1; i < this->nWifim; ++i)
        {
            installTrafficGenerator(this->wifiStaNode.Get(i), this->wifiApNode.Get(0), this->port++, this->offeredLoad, delay+(i - 1) * delta, simulationTime + 2 + delay, callback);
        }
    }
    else
    {
