            ...
            for step in t:
                ...
                for i, agent in enumerate(agents):
                     if step>(history_length/obs_dim):
                          logger.log_round(obs_list[i], reward, cumulative_reward, info, agent.get_loss(), np.mean(obs_list[i], axis=0)[0], step)
            ...
            with tqdm.trange(steps_per_ep) as t:
                for step in t:
                    ...
                    for l, agent in enumerate(agents):
                        if step>(history_length/obs_dim):
                           logger.log_round(obs_list[l], reward, cumulative_reward, info, agent.get_loss(), np.mean(obs_list[l], axis=0)[0], i*steps_per_ep+step)
                    t.set_postfix(mb_sent=f"{logger.sent_mb:.2f} Mb", curr_speed=f"{logger.current_speed:.2f} Mbps")
...
class EnvWrapper:
    ...
    def _craft_commands(self, params):
        ...
        command = f'{waf_pwd} --run "RLinWiFi-Decentralized-v04'
